...
public sealed class MainForm : Form
{
    ...
    private GroupBox BuildCharacterGroup()
    {
        ...
        _energyRegen = new Label { Text = "", Location = new Point(84, y), AutoSize = true,
            ForeColor = Color.FromArgb(80, 110, 160), Font = new Font("Segoe UI", 7.5f, FontStyle.Italic) };
        g.Controls.Add(_energyRegen);
        y += 16;

        _ticketsText  = StatRow(g, "Tickets:",    ref y, "—");
        _ticketsRegen = new Label { Text = "", Location = new Point(84, y), AutoSize = true,
            ForeColor = Color.FromArgb(80, 110, 160), Font = new Font("Segoe UI", 7.5f, FontStyle.Italic) };
    ...
    {
        const int capW = 72;
        var cap = new Label { Text = caption, Location = new Point(10, y), Width = capW,
            ForeColor = Color.Gray, Font = new Font("Segoe UI", 8.5f) };
        var val = new Label { Text = initial, Location = new Point(capW + 12, y),
                                 ...
                                 out NumericUpDown spin, decimal min, decimal max, decimal val)
    {
        ...
        spin = new NumericUpDown { Location = new Point(sx, y), Width = sw,
            Minimum = min, Maximum = max, Value = Math.Clamp(val, min, max), DecimalPlaces = 0 };
        p.Controls.Add(spin);
        y += 30;
    }
                                  ...
                                  out ComboBox combo, string[] items)
    {
        ...
        p.Controls.Add(new Label { Text = label, Location = new Point(8, y + 3), Width = lw });
        combo = new ComboBox { Location = new Point(cx, y), Width = cw,
    ...
    private static CheckBox Chk(Panel p, string text, ref int y, bool @checked, bool bold = false)
    {
        var cb = new CheckBox
        {
            Text     = text, Location = new Point(8, y), AutoSize = true, Checked = @checked,
    ...
    private static Button SmallBtn(string text, ref int x, int y)
    {
        var b = new Button
        {
            Text = text, Location = new Point(x, y), Size = new Size(64, 26),
    ...
    private void OnLogLine(LogLevel level, string line)
    {
        ...
        if (InvokeRequired) { BeginInvoke(() => OnLogLine(level, line)); return; }
        ...
        if (_logBox.Lines.Length > 550)
        {
            int cut = _logBox.GetFirstCharIndexFromLine(50);
            _logBox.Select(0, cut);
